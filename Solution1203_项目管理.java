 ...
 */
public class Solution1203_项目管理 {
...
//        int count=0;
//        while(count<n){
//            int tmp=dfs(n,group,beforeItems,count,tree,visited);
//            if(count==tmp)
//                return ans;
//            count=tmp;
//        }
//        int index=0;
//        for(int i=0;i<m;i++){
//            int length=tree.get(i).size();
//            for(int j=0;j<length;j++){
//                ans[index]=tree.get(i).get(j);
//                index++;
//            }
//        }
//        return ans;
//    }
//    public int dfs(int n,int[] group, List<List<Integer>> beforeItems,int count,List<List<Integer>> tree,boolean[] visited){
//        int[] hfa=new int[n];
//        for(int i=0;i<n;i++){
//            int length=beforeItems.get(i).size();
//            if(length!=0)
//                hfa[i]=-1;
//            for(int j=0;j<length;j++){
//                int tmp_index=beforeItems.get(i).get(j);
//                if(hfa[tmp_index]!=-1){
//                    hfa[tmp_index]=group[i]+1;
//                }
//            }
//        }
//        for(int i=0;i<n;i++){
//            if(!visited[i]&&hfa[i]>=0){
//                hfa[i]=Math.max(0,hfa[i]-1);
//                if(group[i]!=-1){
//                    tree.get(group[i]).add(i);
//                }else {
//                    tree.get(hfa[i]).add(i);
//                }
//                count++;
//                visited[i]=true;
//                clearFa(n,beforeItems,i);
//            }
//        }
//        return count;
//    }
//    public void clearFa(int n,List<List<Integer>> beforeItems,int fa){
//        for(int i=0;i<n;i++) {
//            int length = beforeItems.get(i).size();
//            for (int j = 0; j < length; j++) {
//                beforeItems.get(i).remove(Integer.valueOf(fa));
//            }
//        }
//    }
    public int[] sortItems(int n, int m, int[] group, List<List<Integer>> beforeItems) {
